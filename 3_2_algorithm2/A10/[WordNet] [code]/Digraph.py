...
def sap(g, aList, bList):    
    ...
    for v in aList:
        a_visited[v] = True
        a_distance[v] = 0
        queue.append((v, 0, a_distance[v]))
        
    for v in bList:
        if(a_visited[v]): return(v, 0)
        b_visited[v] = True
        b_distance[v] = 0
        queue.append((v, 1, b_distance[v]))
    
    adj = g.adj
    
    while (queue):
        v, ab, dist = queue.popleft()
        next_dist = dist + 1
        if (next_dist >= sapLength): break
        for w in adj[v]:
            if(ab == 0):
                if(not a_visited[w]):
                    a_visited[w] = True
                    a_distance[w] = next_dist
                    queue.append((w, 0, next_dist))
                    if(b_visited[w]):
                        total = b_distance[w] + next_dist
                        if(total < sapLength):
                            sapLength = total
                            sapVertex = w
                ...
                if(not b_visited[w]):
                    b_visited[w] = True
                    b_distance[w] = next_dist
                    queue.append((w, 1, next_dist))
                    if(a_visited[w]):
                        total = a_distance[w] + next_dist
                        if(total < sapLength):
                            sapLength = total
                            sapVertex = w
