...
class Segment:
    def __init__(self, x1, y1, x2, y2):
        assert(x1==x2 or y1==y2) # Accept either a Vertical or vertical segment  
...
def sweepLine(segments):
    ...
    for i in segments:
        if(i.isVertical()):
            minPQ_x_seg.put((i.x1, i))
        else:
            minPQ_x_seg.put((i.x1, i))
            minPQ_x_seg.put((i.x2, i))
    
    # minPQ가 빌때까지 while문
    while(minPQ_x_seg.qsize() != 0):
        line_tuple = minPQ_x_seg.get()
        line_seg = line_tuple[1]

        # 수직선이면 LLRB체크 -> 결과에 추가
        if(line_seg.isVertical()):
            intersection_list = LLRB_y.rangeSearch(line_seg.y1, line_seg.y2)
            for i in intersection_list:
                result.append((LLRB_y.get(i), line_seg))
                
        # 수평선이면 기존에 없을때는 LLRB에 추가 (y:key, seg:value) 존재할때는 삭제
        else:
            if(LLRB_y.contains(line_seg.y1)):
                LLRB_y.delete(line_seg.y1)
            else:
                LLRB_y.put(line_seg.y1, line_seg)
