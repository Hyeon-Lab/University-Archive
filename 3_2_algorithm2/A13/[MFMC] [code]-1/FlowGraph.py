...
class BaseballElimination:
    ...
    def isEliminated(self, teamName):
        ...
        result = []
        remainTeam = self.numberOfTeams - 1
        matchNodeNum = remainTeam * (remainTeam - 1) // 2
        totalNodeNum = remainTeam + 2 + matchNodeNum
        win = self.wins[teamId] + self.remaining[teamId]
        ...
        graph = FlowNetwork(totalNodeNum)
        # print(matchNodeNum)
        # print("!!!!!!!!!111111\n", graph)
        # print(teamId)
        curNode = 1
        i=0
        j=1
        while(curNode <= matchNodeNum):
            if(i == teamId):
                i = i + 1
                j = j + 1
            else:
                while(j < self.numberOfTeams):
                    if(j == teamId):
                        j = j + 1
                    else:
                        graph.addEdge(FlowEdge(0, curNode, self.against[i][j]))
                        # print(i, j)
                        # print(0, curNode, self.against[i][j])
                        if(i > teamId):
                            graph.addEdge(FlowEdge(curNode, matchNodeNum + i, float('inf')))
                            # print(curNode, matchNodeNum + i, float('inf'))
                        else:
                            graph.addEdge(FlowEdge(curNode, matchNodeNum + 1 + i, float('inf')))
                            # print(curNode, matchNodeNum + 1 + i, float('inf'))
                        if(j > teamId):
                            graph.addEdge(FlowEdge(curNode, matchNodeNum + j, float('inf')))
                            # print(curNode, matchNodeNum + j, float('inf'))
                        else:
                            graph.addEdge(FlowEdge(curNode, matchNodeNum + 1 +j, float('inf')))
                            # print(curNode, matchNodeNum + 1 + j, float('inf'))
                        curNode = curNode + 1
                        j = j + 1
                i = i + 1
                j = i + 1
        ...
        i = 0
        # print("!!!!!!!!!222222\n", graph)
        while(curNode < totalNodeNum - 1):
            if(i != teamId):
                graph.addEdge(FlowEdge(curNode, totalNodeNum - 1, win - self.wins[i]))
                # print(curNode, totalNodeNum - 1, win - self.wins[i])
                curNode = curNode + 1
                i = i + 1
            else:
                i = i + 1
        ...
        ff = FordFulkerson(graph, 0, totalNodeNum - 1)
        # print("!!!!!!!!!44444444\n", graph)
        curNode = matchNodeNum + 1
        for i in range(remainTeam):
            if(ff.inCut(curNode + i)):
                if(i < teamId):
                    result.append(self.teams[i])
                else:
                    result.append(self.teams[i + 1])
