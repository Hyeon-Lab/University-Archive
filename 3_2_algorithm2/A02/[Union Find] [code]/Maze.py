...
def simulate(n, t):
    ...
    for _ in range(t):
        ...
        for i in range(numCells - 1):
            if((i + 1) % n != 0):
                wallsToOpen.append((i, i + 1))
            if(i + n < numCells):
                wallsToOpen.append((i, i + n))
        random.shuffle(wallsToOpen)
        openedWalls = []
        openNum = 0
        ids = [i for i in range(numCells)]
        size = [1 for _ in range(numCells)]

        while(connected(0, numCells-1) == False):
            p, q = wallsToOpen.pop()
            if(connected(p, q) == False):
                union(p, q)
                openNum += 1
                openedWalls.append((p, q))

        openNumLists.append(openNum / (2 * n * (n - 1)))
    return statistics.mean(openNumLists), statistics.stdev(openNumLists), openedWalls
