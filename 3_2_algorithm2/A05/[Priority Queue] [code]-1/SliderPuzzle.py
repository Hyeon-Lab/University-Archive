...
def solveManhattan(initialBoard):
    ...
    twinPQ = PriorityQueue()
    twinPQ.put((0 + twinBoard.manhattan(), twinBoard, 0, None))
    neighbor_list_twin = []

    while(1):
        currentBoard_tuple = minPQ.get()
        currentTwinBoard_tuple = twinPQ.get()
        if(currentBoard_tuple[1].isGoal()):
            while(currentBoard_tuple != None):
                result_list.insert(0, currentBoard_tuple[1])
                currentBoard_tuple = currentBoard_tuple[3]
            return result_list
        
        elif(currentTwinBoard_tuple[1].isGoal()):
            return None
        
        else:
            
            neighbor_list = currentBoard_tuple[1].neighbors()
            for item in neighbor_list:
                if(currentBoard_tuple[3] == None):
                    minPQ.put((currentBoard_tuple[2] + 1 + item.manhattan(),item, currentBoard_tuple[2] + 1, currentBoard_tuple))
                elif(currentBoard_tuple[3][1] != item):
                    minPQ.put((currentBoard_tuple[2] + 1 + item.manhattan(),item, currentBoard_tuple[2] + 1, currentBoard_tuple))

            neighbor_list_twin = currentTwinBoard_tuple[1].neighbors()
            for item in neighbor_list_twin:
                if(currentTwinBoard_tuple[3] == None):
                    twinPQ.put((currentTwinBoard_tuple[2] + 1 + item.manhattan(),item, currentTwinBoard_tuple[2] + 1, currentTwinBoard_tuple))
                elif(currentTwinBoard_tuple[3][1] != item):
                    twinPQ.put((currentTwinBoard_tuple[2] + 1 + item.manhattan(),item, currentTwinBoard_tuple[2] + 1, currentTwinBoard_tuple))
