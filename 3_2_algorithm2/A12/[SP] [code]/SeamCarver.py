...
class SeamCarver:
    ...
    def findVerticalSeam(self):
        ...
        distTo = [[None] * width for _ in range(height)]
        ...
        for y in range(1, height):
            for x in range(width):
                if(x == 0):
                    ...
                    if(distTo[y-1][x] <= distTo[y-1][x-1] and distTo[y-1][x] <= distTo[y-1][x+1]):
                        min_value = distTo[y-1][x]
                        min_x = x
                    elif(distTo[y-1][x-1] <= distTo[y-1][x+1]):
                        min_value = distTo[y-1][x-1]
                        min_x = x-1
                    else:
                        min_value = distTo[y-1][x+1]
        ...
        result = [0] * height
        result_min = distTo[height-1][0]
        result_x = 0
        for i in range(width):
           if(result_min > distTo[height-1][i]):
               result_min = distTo[height-1][i]
               result_x = i
        
        for y in range(height-1, -1, -1):
            result[y] = result_x
            result_x = edgeTo[y][result_x]
        return result
