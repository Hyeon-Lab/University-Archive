...
class SemanticAnalysis(Visitor):
    ...
    def check(self, progAST: Program):
        ...
        # Start of your code:
        main_decl = self.scopeStack.retrieve("main")
    ...
    def _(self, x: FunDecl):
        ...
        # Start of your code:
        check = self.scopeStack.enter(x.idAST.Lexeme, x)
        if not check:
            self.reporter.reportError(self.errMsg[2], "", x.pos)
    ...
    def _(self, x: FormalParamDecl):
        ...
        # Start of your code:
        check = self.scopeStack.enter(x.astIdent.Lexeme, x)
        if not check:
            self.reporter.reportError(self.errMsg[2], "", x.pos)
            x.type = StdEnvironment.errorType
        # End of your code
        ...
        elif isinstance(x.astType, ArrayType) and x.astType.astType.Tequal(StdEnvironment.voidType):
    ...
    def _(self, x: AssignStmt):
        ...
        if x.rAST.type.AssignableTo(x.lAST.type):
            ...
            self.reporter.reportError(self.errMsg[6], "", x.pos)
        # End of your code 
    ...
    def _(self, x: IfStmt):
        ...
        # Start of your code:
        if not x.eAST.type.Tequal(StdEnvironment.boolType):
            self.reporter.reportError(self.errMsg[20], "", x.eAST.pos)
        # End of your code
    ...
    def _(self, x: WhileStmt):
        ...
        # Start of your code:
        if not x.eAST.type.Tequal(StdEnvironment.boolType):
            self.reporter.reportError(self.errMsg[22], "", x.eAST.pos)
        # End of your code
        x.stmtAST.accept(self)
    ...
    def _(self, x: VarDecl):
        ...
        if not isinstance(x.eAST, EmptyExpr):
            ...
            if isinstance(x.tAST, ArrayType):
                ...
                # Start of your code:
                if not isinstance(x.eAST, ExprSequence):
                    self.reporter.reportError(self.errMsg[15], "", x.eAST.pos)
                else:
                    arrType = self.typeOfArrayType(x.tAST)
                    elems = x.eAST
                    count = 0
                    while isinstance(elems, ExprSequence):
                        elemType = elems.lAST.type
                        if not elemType.AssignableTo(arrType):
                            self.reporter.reportError(self.errMsg[13], "", elems.lAST.pos)
                        if arrType.Tequal(StdEnvironment.floatType) and elemType.Tequal(StdEnvironment.intType):
                            elems.lAST = self.i2f(elems.lAST)
                        count += 1
                        elems = elems.rAST
                    if not isinstance(elems, EmptyExpr):
                        if not elems.type.AssignableTo(arrType):
                            self.reporter.reportError(self.errMsg[13], "", elems.pos)
                        if arrType.Tequal(StdEnvironment.floatType) and elems.type.Tequal(StdEnvironment.intType):
                            x.eAST.rAST = self.i2f(elems)
                        count += 1
                    if isinstance(x.tAST.astExpr, IntExpr) and count > int(x.tAST.astExpr.astIL.Lexeme):
                ...
                elif x.tAST.Tequal(StdEnvironment.floatType) and x.eAST.type.Tequal(StdEnvironment.intType):
                    x.eAST = self.i2f(x.eAST)
                # End of your code
        ...
        if x.tAST.Tequal(StdEnvironment.voidType):
            self.reporter.reportError(self.errMsg[3], "", x.pos)
        elif isinstance(x.tAST, ArrayType) and x.tAST.astType.Tequal(StdEnvironment.voidType):
            self.reporter.reportError(self.errMsg[4], "", x.pos)
        # End of your code
    ...
    def _(self, x: VarExpr):
        ...
        if isinstance(x.Ident.declAST, FunDecl):
            self.reporter.reportError(self.errMsg[11], "", x.pos)
            x.type = StdEnvironment.errorType
         # End of your code
    ...
    def _(self, x: UnaryExpr):
        ...
        # Start of your code:
        if self.HasIntOrFloatArgs(x.oAST):
            if x.eAST.type.Tequal(StdEnvironment.intType) or x.eAST.type.Tequal(StdEnvironment.floatType):
        ...
        x.oAST.type = StdEnvironment.errorType
        x.type = StdEnvironment.errorType
        if not isinstance(x.eAST.type, ErrorType):
            self.reporter.reportError(self.errMsg[10], "", x.pos)
        # End of your code 
    ...
    def _(self, x: CallExpr):
        ...
        for i in range(1, NrFormalParams+1):
            ...
            if ActualT.AssignableTo(FormalT):
                if FormalT.Tequal(StdEnvironment.floatType) and \
                ActualT.Tequal(StdEnvironment.intType):
                    Act.pAST = self.i2f(Act.pAST)
            else:
                self.reporter.reportError(self.errMsg[25], "", Act.pAST.pos)
        # End of your code
