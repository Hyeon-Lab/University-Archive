...
def run_test(minic_path, test_input_path, test):
    try:
        ...
        jasmin_fp = open(test+'.j', 'r')
        j_result = jasmin_fp.read()
        return j_result.strip().splitlines()
...
def grade_all_tests(minic_path, testcase_dir='tst/base/testcases', solution_dir='tst/base/solutions'):
    ...
    cases=['Assignment', 'Coercion', 'euclid', 'factorial', 'fibonacci', 'FunctionCall', 'HelloWorld', 'If', 'Intliteral', 'Literals', 'Shortcircuit']
    # You can set the size of testcases (please add your testcases!)
    for i in range(len(cases)):
        test_filename = f'{cases[i]}.mc'
        solution_filename = f'{cases[i]}.j'
        ...
        student_output = run_test(minic_path, test_file, cases[i])
