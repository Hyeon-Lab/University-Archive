...
class Library{
	...
	String publisher;
	int year;
	public Library(String name, String publisher, int year) {
...
public class ex02 {
	public static void main(String[] args) {
		...
		int index = -1;
		while(n != 5) {
			System.out.print("삽입:1, 삭제:2, 찾기:3, 전체:4, 종료:5 -->");
			n = s.nextInt();
			switch(n) {
			case 1:
				...
				if(index == -1) {
					Library l = new Library(name, publisher, year);
					a.add(l);
				}
				else {
					index = -1;
			...
			case 3:
				...
				if(index != -1) {
					System.out.println(name + "값은 " + index + "번째 위치");
					index = -1;
				}
				break;
			case 4:
